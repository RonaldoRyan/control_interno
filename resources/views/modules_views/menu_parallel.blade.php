...
{{-- inicio de botones que invocan el modal para la creacion de perfiles o fichas --}}
    ...
    @can('saveStudent')
    <li class="nav-item">
    <button type="submit" class="btn crearPerfil" id="modalStudent" data-toggle="modal" data-target="#student">{{ __('Crear ficha de Estudiante') }}</button>
    </li>
    @endcan
    ...
    @can('saveOtherWorker')
    <li class="nav-item">
    <button type="button" class="btn crearPerfil" id="modalOthers" data-toggle="modal" data-target="#other">{{ __('Otro tipo de Ficha') }}</button>
    </li>
    @endcan
...
<form id="crearPerfilForm" method="get" action="{{ route('sections', ['option' => 'students']) }}">
    ...
    <button type="submit" class="btn panelRegistro">{{ __('Panel De Registro de Estudiantes') }}</button>
</form>
</li>
...
<form id="crearPerfilForm" method="get" action="{{ route('sections', ['option' => 'professors']) }}">
    ...
    <button type="submit" class="btn panelRegistro">{{ __('Panel De Registro de profesores') }}</button>
</form>
</li>
...
<form id="crearPerfilForm" method="get" action="{{ route('sections', ['option' => 'othersWorkers']) }}">
    ...
    <button type="submit" class="btn panelRegistro">{{ __('Panel De Registro de Otros Funcionarios') }}</button>
</form>
</li>
@endcan
...
<form id="crearPerfilForm" method="get" action="{{ route('sections', ['option' => 'mealBenefit']) }}">
    ...
    <button type="submit" class="btn  panelRegistro">{{ __('Panel de Registro de Beneficiarios de servicio de comedor') }}</button>
</form>
</li>
...
<div class="collapse menu-btn main-btn" id="menu3">
<ul class="list-group">
    <li class="nav-item">
        <form id="crearPerfilForm" method="get" action="{{ route('sections', ['option' => 'babyGroup']) }}">
            <input type="hidden" name="tipo" value="babyGroup">
            <button type="submit" class="btn panelRegistro">{{ __('Grupo de 0 a 2 años') }}</button>
        </form>
        </li>
    ...
    <form id="crearPerfilForm" method="get" action="{{ route('sections', ['option' => 'middleGroup']) }}">
        ...
        <button type="submit" class="btn panelRegistro">{{ __('Grupo de 3 a 4 años') }}</button>
    </form>
    </li>
    ...
    <form id="crearPerfilForm" method="get" action="{{ route('sections', ['option' => 'olderGroup']) }}">
        ...
        <button type="submit" class="btn panelRegistro">{{ __('Grupo de 5 a 6 años') }}</button>
    </form>
    </li>
</ul>
</div>
